  ...
  return (
    <AppShell>
      ...
      <div className="flex-1 p-8 space-y-6">
        ...
        <StaggerContainer className="grid grid-cols-6 gap-3">
          {(Object.keys(catLabel) as (DocCategory | 'todos')[]).map((cat) => {
            const count = cat === 'todos'
              ? myDocuments.length
              : myDocuments.filter((d) => d.category === cat).length
            return (
              <StaggerItem key={cat}>
                <button
        ...
        {/* Document list — re-animates on filter/search change via key */}
        <StaggerContainer key={`${catFilter}-${search}`} className="space-y-2">
          {filtered.map((doc) => (
